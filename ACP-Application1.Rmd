...
La fonction `get\_pca\_var()` permet d'extraire les résultats relatives aux variables d'origines depuis l'objet retourné par la fonction `PCA`     
Appliquer la sur l'objet `decath.acp`    Nommer l'output `cmp\_var`  
Afficher les élément de l'objet `cmp\_var` par la fonction `str()`
...
Les élément de l'objet `cmp\_var` qui serviront à la représentation des variables sont : 
